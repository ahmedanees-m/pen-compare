"""Unified Editor Universe assembly.
...
def _build_natural_row(editor, scored, metadata, atlas_present: bool) -> dict:
    ...
    # S_DSB fallback: if mech-class raw-data models are not yet available,
    ...
    return {
        ...
        "length_aa": None,  # not in EditorEntry schema; added in a later annotation pass
        "mechanism_class": editor.mechanism_bucket,
        "tier_a_gate": tier_a,
        "mech_class_confidence": None,  # populated once mech-class raw-data models are available
        "s_dsb": s_dsb,
